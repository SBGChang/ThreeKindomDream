...
> **狀態**：全部 33 個模組的契約與 Data Schema 已定義。實作尚未開始
>
> 🔧 **[RFC-01 大檢定改為戰役制](docs/RFC-01-campaign-rework.md) 為提案中、尚未實作。**
> 它新增模組 ㉜㉝、重寫 ㉓、縮編 ⑱，並推翻本文件多處已校準的數值。
> 受影響的模組契約頂端都有 🔧 標記。
...
| 18 | **檢定引擎** | 小檢定＋大檢定共用、DC、成功率、難度選擇 | 🔵 ／ ✂️ RFC-01 縮編為只有小檢定 |
| 19 | **名士局內狀態** | 3+3 陣容組建、好感度、連動、事件鏈進度 | 🔵 |
| 20 | **屬性與貨幣** | 四維、名聲（文／武／善惡）、功績（文／武） | 🔵 |
| 21 | **官階系統** | 文武雙軌、升遷檢定、作為委託門檻 | 🔵 |
| 22 | **陣營系統** | 選擇、資格門檻、專屬事件池、主公 | 🔵 |
| 23 | **特質與技能** | 特質＝常駐被動；技能＝戰役中的行動；解鎖與事件分支 | ♻️ RFC-01 已重寫 |
| 24 | **寶物局內狀態** | 持有、獲得、**本輪重複判定**、碎片產出 | 🔵 |
| **32** | **養成兌現** | 四類經驗池、三張消耗表、解鎖清單；**經驗 → 能力的唯一兌換點** | 🆕 RFC-01 提案 |
| **33** | **戰役** | 章末七關自動戰役、軍勢與糧秣、施放序列、走留決策 | 🆕 RFC-01 提案 |
...
| [RFC-01 · 大檢定改為戰役制](docs/RFC-01-campaign-rework.md) | **提案中。** 決議記錄（D1–D40）＋ 模組配置 ＋ 四階段遷移順序 ＋ 待校準數字與其反推依據 |
