...
>
> ✅ **[RFC-01 大檢定改為戰役制](docs/RFC-01-campaign-rework.md) 的階段 A–C 已實作。**
> 它新增模組 ㉜㉝、重寫 ㉓、縮編 ⑱，並推翻本文件多處已校準的數值。
> **階段 D 校準未做**，且有一個未解的平衡問題（RFC §9.2：貪心目前不划算）。
...
| 23 | **特質與技能** | 特質＝常駐被動；技能＝戰役中的行動；解鎖與事件分支 | 🔵 已實作 |
| 24 | **寶物局內狀態** | 持有、獲得、**本輪重複判定**、碎片產出 | 🔵 |
| **32** | **養成兌現** | 四類經驗池、三張消耗表、解鎖清單；**經驗 → 能力的唯一兌換點** | 🔵 已實作 |
| **33** | **戰役** | 章末七關自動戰役、軍勢與糧秣、施放序列、走留決策 | 🔵 已實作 |
...
| [RFC-01 · 大檢定改為戰役制](docs/RFC-01-campaign-rework.md) | **階段 A–C 已實作。** 決議記錄（D1–D40）＋ 模組配置 ＋ 四階段進度 ＋ 實測結果與未解的平衡問題（§9） |
