...
export const TACTIC_VFX:Readonly<Record<string,number>>={pincer:13,cavalry:11,longshot:12,infighting:10,misreport:10,fire:0,inspire:13,shield:6,reform:9,toarcher:14,toinfantry:14,pursue:12,mounted:12,volley:12,crossbow:12,thunder:4,water:0,rocks:2,taunt:11,ambush:10,reinforce:13,divide:14,turncoat:14,lure:10,sweep:7};
...
export function drawTacticalParticles(ctx:CanvasRenderingContext2D,atlas:Atlas,c:Cinematic,s:BattleState):void{
 ...
 if(['water','rocks','thunder'].includes(m)){
  if(!impact){
   for(let i=0;i<(m==='water'?4:5);i++){
    ...
    effectStamp(ctx,atlas,cell,x,y,m==='water'?150:85,m==='thunder'?230:150,clamp(p*3));
   }
...
export function drawStatusParticles(ctx:CanvasRenderingContext2D,atlas:Atlas,s:BattleState,fireAtlas?:Atlas):void{
 const traits:Record<string,number>={frugal:15,breakline:7,hunters:12,aftershock:13,sighting:12,supply:15,exploit:7,backwater:6,fervor:13,relief:9,laststand:6,drill:14};
