...
const PREP_TABS = [{ id: 'skills', name: '主將招式' }, { id: 'commanders', name: '同行指揮' }, {id:'equipment',name:'戰場配裝'}] as const;
...
export function ScreenCampaign({ s, bump, onDepart, onLearn }: Props): React.ReactElement {
  ...
  return <section className="campaign-prep" aria-label="出戰整備"><div inert={picker !== null}>
    ...
    <div className={'prep-desk prep-tab-' + tab}>
      ...
      </section>    <section className="prep-equipment prep-panel" role="tabpanel" id="prep-panel-equipment" aria-labelledby="prep-tab-equipment" hidden={tab !== 'equipment'}>{(['weapon','mount','treasure'] as const).map(slot=><label key={slot}>{slot==='weapon'?'武器':slot==='mount'?'坐騎':'寶物'}<select aria-label={slot==='weapon'?'裝備武器':slot==='mount'?'裝備坐騎':'裝備寶物'} value={s.current.equipment?.[slot]??''} onChange={e=>{s.equipItem((e.target.value||null) as import('../contracts/core/ids.js').ItemId|null,slot);bump();}}><option value="">不啟用</option>{s.equipmentOptions().filter(d=>d.equipment?.slot===slot).map(d=><option key={d.itemId} value={d.itemId}>{t(d.nameKey)}：{t(d.descKey)}</option>)}</select></label>)}</section></div>
