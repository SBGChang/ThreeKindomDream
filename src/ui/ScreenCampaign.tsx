...
export function ScreenCampaign({ s, bump, onDepart, onLearn }: Props): React.ReactElement {
  ...
  return <section className="campaign-prep" aria-label="出戰整備"><div inert={picker !== null}>
    ...
    <aside className="prep-route" aria-label="行軍路線"><h2>行軍路線</h2><p>累計獎金</p><ol>{rows.map(row => <li key={row.index} className={row.boss ? 'has-general' : ''}><span className="prep-route-no">{row.index + 1}</span><div><b>{row.boss ? t(row.boss.nameKey) : '第 ' + (row.index + 1) + ' 關'}{row.unique && <span className="prep-drop" title="本關含特殊掉落" aria-label="本關含特殊掉落">◆</span>}</b><small>{number(row.cumulative)}<span> 錢</span></small></div>{row.boss && <CharacterArt name={t(row.boss.nameKey)} portrait />}</li>)}</ol></aside>
