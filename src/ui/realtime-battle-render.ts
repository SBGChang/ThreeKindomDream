...
function troop(ctx:CanvasRenderingContext2D,images:BattleImages,u:Troop,s:BattleState,opacity:number,retreat=false){
 ...
 sprite(ctx,images,prefix+name,f,x,y,size,retreat?!enemy:enemy,alpha,filter);
}
function skillCamera(c:Cinematic|null):{x:number;y:number;zoom:number} {
 ...
 if(k==='fire')x=t<1.35?360:mix(360,c.targetX,ease((t-1.35)/.85));
...
function cinematicArt(ctx:CanvasRenderingContext2D,images:BattleImages,c:Cinematic){
 ...
 if(k==='fire'){
  ...
  const atlas=images['fire-particles'];if(atlas)drawFireParticles(ctx,atlas,t,c.targetX,c.targetY);
 }
 if(k==='pincer'){
...
/** Stage the existing unit at the rear center, with no duplicate or translucent cut-in. */
export function stageSkillCommander(g:Commander,s:BattleState,cam:{x:number;y:number;zoom:number}):{unit:Commander;opacity:number;size:number} {
 const c=s.cinematic;if(!c)return {unit:g,opacity:1,size:185};
 const arrive=ease(c.time/.5),leave=ease((c.time-3.6)/.6),weight=arrive*(1-leave);
 if(g.name!==c.skill.owner)return {unit:g,opacity:clamp(1-ease(c.time/.3)+leave),size:185};
 const moving=c.time<.5||c.time>3.6;
 const unit:Commander={...g,x:mix(g.x,cam.x,weight),y:mix(g.y,cam.y-110/cam.zoom,weight),pose:moving?'move':'command',poseTime:moving?c.time:Math.max(0,c.time-.5),flip:c.time>3.6?g.x<cam.x:false};
 return {unit,opacity:1,size:185/mix(1,cam.zoom,weight)};
}
export function drawBattle(ctx:CanvasRenderingContext2D,images:BattleImages,s:BattleState,presentation:{armyOpacity?:number;retreatSide?:'ally'|'enemy';camera?:{x:number;y:number;zoom:number}}={}):void {
 ...
 const skillAtlas=images['skill-particles'];
 if(local&&skillAtlas)drawSkillParticles(ctx,skillAtlas,local,'rear',s.units);
 layers.sort((a,b)=>a.y-b.y).forEach(layer=>layer.draw());
 if(local&&local.time>=0)cinematicArt(ctx,images,local);
 if(local&&skillAtlas)drawSkillParticles(ctx,skillAtlas,local,'front',s.units);
 ctx.restore();
