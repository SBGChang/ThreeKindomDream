...
export function EventChallengeView({session:s,onChange,onDone}:{session:Session;onChange:()=>void;onDone:()=>void}):React.ReactElement {
 ...
 useEffect(()=>{if(state.phase!=='playing'||state.rally)return;let alive=true;setReady(false);void Promise.all([loadBattleImages(state.contest?"/art/backgrounds/bg-drill.png":undefined,state.battle?.commanders??[]),loadDuelImages(['lord',state.definition.opponent])]).then(([a,b])=>{if(alive){images.current={...a,...b};setReady(true);}}).catch(e=>{if(alive)setError(String(e));});return()=>{alive=false;};},[state.phase]);
 ...
 if(state.phase==='playing'&&state.rally)return <div className="event-challenge ec-playing"><RallyDebateDemo campaign={{state:state.rally,enemyName:state.definition.opponentName,initialPaused:state.paused,returnLabel:'繼續對話',background:'/art/backgrounds/bg-study.png',onRetreat:()=>act(()=>s.retreatEventChallenge()),onPause:p=>act(()=>s.pauseEventChallenge(p)),onAction:(side,a)=>{const ok=s.answerEventDebate(side,a);sync();return ok;},onDone:()=>act(()=>s.finishEventDebate())}}/></div>;
 ...
 if(state.phase==='prep'){
 const learned=s.current.abilities.skills,visible=learned.slice(skillPage*6,skillPage*6+6);
 ...
 const result=state.phase==='result',text=intermission?stage!.victory:result?state.definition.outcomes[state.outcome!]+(state.outcome==='retreat'&&stage?'\n獎勵：獲得 '+(state.definition.cashOutGold?.[state.completed??0]??0)+' 金。':''):stage?.opening??state.definition.opening,lines=text.split('\n'),line=lines[Math.min(page,lines.length-1)]!,colon=line.indexOf('：'),speaker=colon>=0?line.slice(0,colon):result?'戰後':'你',body=colon>=0?line.slice(colon+1):line;
 return <div className="event-challenge ec-conversation" style={{backgroundImage:`url(/art/backgrounds/${state.definition.mode==='debate'?'bg-study':state.definition.mode==='duel'?'bg-drill':'bg-battle-1'}.png)`}}><div className="dialogue-cast"><DialogueActor profile={careerPresentation('war',s.current.career)} actor={{name:state.definition.opponentName}} index={1} home={{x:73,y:0}} cue={String(page)} speaking={speaker!=='你'} reduced={false}/><DialogueActor profile={careerPresentation('war',s.current.career)} actor={{name:'你',hero:true}} index={0} home={{x:27,y:0}} cue={String(page)} speaking={speaker==='你'} reduced={false}/></div><section className="conversation-box"><div className="conversation-content"><div className="dialogue-speaker-name">{speaker}</div><p className="dialogue-line"><DialogueText text={body}/></p></div><button className="ec-control ec-next" onClick={()=>{if(page+1<lines.length)setPage(page+1);else{setPage(0);if(result)onDone();else if(intermission)act(()=>s.continueEventChallenge());else act(()=>s.enterEventChallenge());}}}>{page+1<lines.length?'繼續':result?'記下此事':intermission?'繼續挑戰':state.definition.mode==='battle'?'整備出陣':'請賜教'}</button>{stage&&!result&&<button className="ec-control ec-cashout" onClick={()=>{setPage(0);act(()=>s.retreatEventChallenge());}}>放棄挑戰・領取 {state.definition.cashOutGold?.[state.completed??0]??0} 金</button>}</section></div>;
}
