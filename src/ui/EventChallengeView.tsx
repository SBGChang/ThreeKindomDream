...
export function EventChallengeView({session:s,onChange,onDone}:{session:Session;onChange:()=>void;onDone:()=>void}):React.ReactElement {
 ...
 return <div className="event-challenge ec-conversation" style={{backgroundImage:`url(/art/backgrounds/${stageChallenge(state).mode==='debate'?'bg-study':stageChallenge(state).mode==='duel'?'bg-drill':'bg-battle-1'}.png)`}}><div className="dialogue-cast"><DialogueActor profile={careerPresentation('war',s.current.career)} actor={{name:stageChallenge(state).opponentName}} index={1} home={{x:73,y:0}} cue={String(page)} speaking={speaker!=='你'} reduced={false}/><DialogueActor profile={careerPresentation('war',s.current.career)} actor={{name:'你',hero:true}} index={0} home={{x:27,y:0}} cue={String(page)} speaking={speaker==='你'} reduced={false}/></div><section className="conversation-box"><div className="conversation-content"><div className="dialogue-speaker-name">{speaker}</div><p className="dialogue-line"><DialogueText text={body}/></p></div><button className="ec-control ec-next" onClick={()=>{if(page+1<lines.length)setPage(page+1);else{setPage(0);if(result)onDone();else if(intermission)act(()=>s.continueEventChallenge());else act(()=>s.enterEventChallenge());}}}>{page+1<lines.length?'繼續':result?'記下此事':intermission?'繼續挑戰':stageChallenge(state).mode==='battle'?'整備出陣':'請賜教'}</button>{state.canDecline&&state.phase==='opening'&&<button className="ec-control ec-cashout" onClick={()=>{setPage(0);act(()=>s.retreatEventChallenge());}}>改日再論</button>}{stage&&!result&&<button className="ec-control ec-cashout" onClick={()=>{setPage(0);act(()=>s.retreatEventChallenge());}}>放棄挑戰・領取 {state.definition.cashOutGold?.[state.completed??0]??0} 金</button>}</section></div>;
}
