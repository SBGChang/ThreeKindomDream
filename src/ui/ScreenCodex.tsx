...
export function ScreenItemCodex({meta,onBack}:Props):React.ReactElement {
 ...
 return <section className="realm realm-treasury"><header className="realm-heading"><div><RealmIcon name="chest"/><h1>天工閣</h1></div><span>藏品 {rows.length} / {all.length}</span><RealmBack onClick={onBack}/></header>{it?<div className="treasury-layout"><div className="treasury-shelves">{rows.map(r=><button key={String(r.itemId)} className={r===it?'selected':''} aria-pressed={r===it} onClick={()=>setSelected(String(r.itemId))}><ItemArt name={t(r.nameKey)} className="relic-art"/><b>{t(r.nameKey)}</b><RealmStars count={itemCodex.tierOf(r.itemId,meta)+1}/></button>)}</div><article className="relic-detail"><ItemArt name={t(it.nameKey)} className="relic-art large"/><h2>{t(it.nameKey)}</h2><RealmStars count={it.rarity}/><Frag have={itemCodex.entry(it.itemId,meta).fragments} need={itemCodex.nextCost(it.itemId,meta,defs)}/><ul>{it.tiers.map(row=><li key={row.tier} className={itemCodex.unlockedTiers(it.itemId,meta,defs).some(o=>o.tier===row.tier)?'':'sealed'}>{t(row.descKey)}</li>)}</ul></article></div>:<div className="treasury-empty"><RealmIcon name="chest" className="vault-art"/><h2>尚未收藏</h2><p>在夢中獲得器物，即可收藏於此。</p></div>}</section>;
}
