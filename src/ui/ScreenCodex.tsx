...
export function ScreenNotableCodex({meta,onBack}:Props):React.ReactElement {
 ...
 return <section className="realm realm-heroes"><header className="realm-heading"><div><RealmIcon name="book"/><h1>風雲錄</h1></div><RealmBack onClick={onBack}/></header><div className="hero-book"><nav className="hero-index" aria-label="名士史冊">{rows.map(r=><button className={r===n?'selected':''} aria-pressed={r===n} key={String(r.notableId)} onClick={()=>setSelected(String(r.notableId))}><OfficerPortrait name={t(r.nameKey)}/><span>{t(r.nameKey)}<RealmStars count={notableCodex.starOf(r.notableId,meta)} total={notableCodex.maxStar(defs)}/></span></button>)}</nav>{n&&<article className="hero-folio" key={String(n.notableId)}><div className="hero-illustration"><CharacterArt name={t(n.nameKey)}/></div><div className="hero-biography"><h2>{t(n.nameKey)}</h2><RealmStars count={notableCodex.starOf(n.notableId,meta)} total={notableCodex.maxStar(defs)}/><Frag have={notableCodex.entry(n.notableId,meta).fragments} need={notableCodex.nextCost(n.notableId,meta,defs)}/><h3>羈絆傳承</h3><ul>{notableCodex.unlockedRows(n.notableId,meta,defs).map(u=><li key={String(u.descKey)}>{t(u.descKey)}</li>)}</ul>{notableCodex.unlockedRows(n.notableId,meta,defs).length===0&&<p>升星解鎖傳承</p>}</div></article>}</div></section>;
}
export function ScreenItemCodex({meta,onBack}:Props):React.ReactElement {
 ...
 return <section className="realm realm-treasury"><header className="realm-heading"><div><RealmIcon name="chest"/><h1>天工閣</h1></div><span>藏品 {rows.length} / {all.length}</span><RealmBack onClick={onBack}/></header>{it?<div className="treasury-layout"><div className="treasury-shelves">{rows.map(r=><button key={String(r.itemId)} className={r===it?'selected':''} aria-pressed={r===it} onClick={()=>setSelected(String(r.itemId))}><RealmIcon relic name={t(r.nameKey)} className="relic-art"/><b>{t(r.nameKey)}</b><RealmStars count={itemCodex.tierOf(r.itemId,meta)+1}/></button>)}</div><article className="relic-detail"><RealmIcon relic name={t(it.nameKey)} className="relic-art large"/><h2>{t(it.nameKey)}</h2><RealmStars count={it.rarity}/><Frag have={itemCodex.entry(it.itemId,meta).fragments} need={itemCodex.nextCost(it.itemId,meta,defs)}/><ul>{it.tiers.map(row=><li key={row.tier} className={itemCodex.unlockedTiers(it.itemId,meta,defs).some(o=>o.tier===row.tier)?'':'sealed'}>{t(row.descKey)}</li>)}</ul></article></div>:<div className="treasury-empty"><RealmIcon name="chest" className="vault-art"/><h2>尚未收藏</h2><p>在夢中獲得器物，即可收藏於此。</p></div>}</section>;
}
