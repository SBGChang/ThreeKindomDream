...
export function ItemEffects({ rows, compact = false }: { rows: readonly string[]; compact?: boolean; artful?: boolean }): React.ReactElement {
  ...
  return <div className={'item-effect-pages '+(compact ? 'compact' : '')}>
    <div>{rows.slice(current*size,current*size+size).map((text,i) => <p key={i}>{<EffectText text={text}/>}</p>)}</div>
