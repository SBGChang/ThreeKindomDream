import {loadDebateImages,debateFrame,DEBATE_MOTION_ROWS} from './debate-art.js';
...
const poses:Record<string,string>={debate:'舌戰',battle:'戰法',command:'指揮',run:'行軍',attack:'單挑攻擊',defend:'單挑防守',rest:'單挑休養',hurt:'單挑受擊',evolution:'單挑升變'};
...
export function BattleEffectsReview():React.ReactElement{
 ...
 const [debate,setDebate]=useState<DebateCard|null>('claim');
 ...
 void Promise.all([loadBattleImages(undefined,b.commanders),loadDuelImages([hero]),loadDebateImages([hero])]).then(([a,d,t])=>{if(alive)setArt({...a,...d,...t});}).catch(e=>{if(alive)setError(String(e));});return()=>{alive=false;};},[hero]);
 ...
 if(mode==='debate'){const im=art['debate-'+hero];if(im){const f=debateFrame(debate,time);ctx.drawImage(im,f%4*320,Math.floor(f/4)*320,320,320,600,255,400,400);}return;}
 ...
 },[art,hero,mode,mechanic,evolution,time,duration,debate]);
 return <main className="rt-shell"><div className="rt-battle dm-review"><header><a href="?art=battle-demo">‹ 演武場</a><h1>戰場美術演武</h1><span>{Object.keys(DUEL_ACTORS).length} 個角色 · 25 種戰法</span><a href="?art=duel-motion">武將總覽</a></header>
 ...
 {mode==='debate'&&<select aria-label="舌戰動作" value={debate??'idle'} onChange={e=>{setDebate(e.target.value==='idle'?null:e.target.value as DebateCard);setTime(0);setPlaying(false);}}>{[...Object.keys(DEBATE_MOTION_ROWS),'idle'].map((id,i)=><option key={id} value={id}>{['義理','事證','質問','反論','借題','集中','怒斥','待機'][i]}</option>)}</select>}
