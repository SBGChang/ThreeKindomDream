import {RallyGuide,SpecialRuleDiagram,specialRuleSummary} from './RallyGuide.js';
...
function Vitals({side,f,value,forecast,comboPreview,displayName,actorId}:{side:RallySide;f:RallyFighter;value:number;forecast:number;comboPreview:boolean;displayName?:string;actorId:string}){
 ...
 return <section className={`dx-fighter db-fighter dx-fighter-${side}`} aria-label={`${enemy?'敵方':'我方'}舌戰狀態`}>
  <div className="dx-face-medallion"><div className="dx-face-image"><DuelPortrait id={actorId} name={name}/></div></div>
...
export function RallyDebateDemo({campaign}:{campaign?:CampaignDebateBinding}={}):React.ReactElement {
 ...
 const locked=!ready||!!error||menu||help||busy||s.turn!=='ally'||!!s.winner;
 ...
 useEffect(()=>{if(!ready)return;let raf=0,previous=performance.now(),ui=0;const reduced=window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const frame=(now:number)=>{const dt=Math.min(.05,(now-previous)/1000);previous=now;const c=controls.current,m=motion.current,d=model.current;
   ...
   const ctx=canvas.current?.getContext('2d');if(ctx&&images.current)drawRally(ctx,images.current,d,m.time,m.active,reduced,actors);
   if(now-ui>50){setTime(m.time);ui=now;}raf=requestAnimationFrame(frame);
  };raf=requestAnimationFrame(frame);return()=>cancelAnimationFrame(raf);
 },[ready,actors.ally,actors.enemy]);
 ...
 return <main className="rt-shell"><div className="rt-battle ct-battle cb-battle rally-battle" data-paused={help} data-phase={menu?'setup':busy?'animation':s.winner?'finished':s.turn}>
  ...
  {!menu&&<><Vitals actorId={actors.ally} displayName={campaign?'你':rallyProfile(actors.ally).name} side="ally" f={s.ally} value={showBefore&&event?event.before.ally:s.ally.heart} forecast={showBefore&&event?event.after.ally:preview?.ally??s.ally.heart} comboPreview={!!preview&&s.lastPass==='enemy'}/><Vitals actorId={actors.enemy} displayName={campaign?.enemyName??rallyProfile(actors.enemy).name} side="enemy" f={s.enemy} value={showBefore&&event?event.before.enemy:s.enemy.heart} forecast={showBefore&&event?event.after.enemy:preview?.enemy??s.enemy.heart} comboPreview={false}/>
   ...
   {inspectedSpecial&&<aside className="rally-special-inspect" id="rally-special-inspect" role="tooltip" aria-label={`${RALLY_SPECIALS[inspectedSpecial.special].name}效果說明`}>
    ...
    <div className="rally-inspect-scroll"><img src="./art/duel/status-scroll-v1.png" alt=""/><div className="rally-inspect-copy"><h2>{RALLY_SPECIALS[inspectedSpecial.special].name}</h2><SpecialRuleDiagram special={inspectedSpecial.special}/><p>{specialRuleSummary(inspectedSpecial.special)}</p></div></div>
   </aside>}
