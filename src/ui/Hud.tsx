...
export interface GrowthView { values: Record<Attr,number>; learning:number }
export const captureGrowth=(s:Session):GrowthView=>({values:{...s.current.attributes.values},learning:s.money});
...
export function Hud({ s, onLearn, onVault, growth, preview, meritPreview }: { readonly s: Session; readonly onLearn?: () => void; readonly onVault?: () => void; readonly preview?: Partial<Record<Attr,number>> | undefined; readonly meritPreview?: Partial<Record<CareerLine,number>> | undefined; readonly growth?:GrowthView | undefined }): React.ReactElement {
  ...
  return <section className="player-panel" aria-label="本輪能力與官途">
    <div className="player-panel-title"><span>此生修為</span><span className="learning-total">金錢 <b>{growth?.learning ?? s.money}</b></span></div>
