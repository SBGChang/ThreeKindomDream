...
export function GameFrame({ meta, session, active, onGo, saveNotice, children, onReturnHome, beforeExit }: {
  ...
  const runChrome = !(session?.needsCampaign && active === 'run') && session !== null && ['run', 'camp', 'learn', 'market', 'vault'].includes(active) && !locked;
