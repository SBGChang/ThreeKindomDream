...
function Scene({title,note,children,wide=false}:{title:string;note:string;children:ReactNode;wide?:boolean}){return <article className={`rg-scene ${wide?'rg-wide':''}`}><h3>{title}</h3><div className="rg-picture">{children}</div><p>{note}</p></article>;}
...
export function RallyGuide({onClose}:{onClose:()=>void}){
 ...
 return <div className="ct-overlay rg-overlay"><section ref={panel} className="rg-book" role="dialog" aria-modal="true" aria-label="舌戰圖解" onKeyDown={e=>{if(e.key==='Escape'){e.stopPropagation();onClose();}if(e.key==='Tab'){const buttons=Array.from(panel.current?.querySelectorAll<HTMLButtonElement>('button')??[]),index=buttons.indexOf(document.activeElement as HTMLButtonElement);if(e.shiftKey&&index===0){e.preventDefault();buttons.at(-1)?.focus();}else if(!e.shiftKey&&index===buttons.length-1){e.preventDefault();buttons[0]?.focus();}}}}>
  ...
  </div>:<div className="rg-specials">{specials.map(sp=><article key={sp.id}><RallyCard interactive={false} card={{id:0,kind:'special',special:sp.id}}/><h3>{sp.note}</h3><div className="rg-special-scene">{sp.id==='induct'?<><Card value={2}/><Card color="evidence" value={3}/><Arrow/><Card value={5}/><Card color="evidence" value={5}/></>:sp.id==='concentrate'?<Draw/>:sp.id==='wild'?<><Card value={9}/><Arrow/><Card color="presence" value={2}/></>:<><DebateIcon card={sp.id==='shout'?'pressure':'rebut'}/><span className="rg-effect">{sp.id==='shout'?'雙倍':'反彈'}</span></>}</div><p>{sp.detail}</p></article>)}</div>}
  </div>
  <footer>{tab==='basic'?'Hover 看預估　・　點牌就出　・　有牌必須出':'特殊卡用後補 1 張普通牌　・　每回合限 1 張，仍可出普通牌'}</footer>
 </section></div>;
}
