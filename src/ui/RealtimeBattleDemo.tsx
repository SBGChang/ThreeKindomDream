...
export function RealtimeBattle({campaign,bump,onDone}:{campaign?:Session;bump?:()=>void;onDone?:()=>void}={}):React.ReactElement {
 ...
 useEffect(()=>{let alive=true;void loadBattleImages(campaign?`./art/backgrounds/bg-battle-${Math.min(4,campaign.current.progress.chapter)}.png`:undefined,battle.current.commanders).then(v=>{if(alive){images.current=v;setReady(true);}}).catch(e=>{if(alive)setError(String(e));});return()=>{alive=false;};},[]);
 useEffect(()=>{
 ...
 return <main className={campaign?"rt-shell rt-campaign":"rt-shell"}><div className="rt-battle" data-status={state.status} data-wave-phase={state.phase} data-wave={state.wave} data-cinematic={c?.skill.kind??'none'}>
  <canvas ref={canvas} width={1600} height={900} aria-label={`即時戰場：我軍 ${armyCount(state,'ally')} 人、敵軍 ${armyCount(state,'enemy')} 人`}/>
  <div className="rt-top-left"><span>{campaign?'整軍出陣 · 章末戰役':'大檢定・演武試作'}</span>{!campaign&&<a href="?art=unit-sequence">返回動作預覽</a>}</div>
