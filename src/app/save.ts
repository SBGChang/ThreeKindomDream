import {validEventChallengeSnapshot} from './event-challenge-validation.js';
...
export function restoreRun(w: Wiring): {
  ...
  try {
    ...
    if(!validEventChallengeSnapshot(s))throw Error('事件挑戰存檔格式不符');
    if(s.eventChallenge&&s.eventChallenge.source!=='chapter'&&!w.defs.reader('event').get(s.eventChallenge.eventId).options[s.eventChallenge.option]?.challenge)throw Error('事件挑戰選項已失效');
