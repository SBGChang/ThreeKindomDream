...
export function eventChallengeReview(mode:string):Session {
 const initial=dialogueReview('notable'),d=defs.reader('event').all().find(d=>mode==='nanhua'?d.options.some(o=>o.challenge?.stages&&o.challenge.mode==='debate'):mode==='trial'?d.options.some(o=>o.challenge?.stages&&o.challenge.mode==='duel'):d.trigger.kind==='commission'&&d.options.some(o=>o.challenge?.mode===mode))!;
