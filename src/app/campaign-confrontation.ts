...
export function campaignConfrontation(ctx:RunContext,battle:BattleState):EncounterProgress {
 ...
 const candidates=[protagonist,...(ctx.state.campaign?.loadout?.commanders??[]).map(slot=>{const n=ctx.defs.reader('notable').get(String(slot.notableId));return {id:String(n.nameKey).split('.')[1]!,name:ctx.defs.text(String(n.nameKey)),build:{war:n.abilities.attrs.war,lead:n.abilities.attrs.lead,trait:trait(n.abilities.traits)}};})];
 ...
 const waveOpponents=campaign.stageRows(ctx).map((_,i)=>{const row=campaign.nextStagePreview(ctx,i)!,boss=row.boss,name=boss?ctx.defs.text(String(boss.nameKey)):'陣營指揮官';const build:DuelBuild={war:boss?.attrs.war??generic,lead:boss?.attrs.lead??generic,trait:'none'};return {id:duelActorForName(name),name,build};});
