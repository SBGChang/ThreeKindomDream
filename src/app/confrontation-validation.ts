...
const build=(b:DuelBuild)=>b&&(b.comboEnabled===undefined||typeof b.comboEnabled==='boolean')&&finite(b.war)&&finite(b.lead)&&validDuelBuild(b);
const fighter=(f:DuelFighter)=>f&&(f.progression===undefined||typeof f.progression==='boolean')&&[f.stamina,f.maxStamina,f.injury,f.injuryLimit,f.attack,f.defense,f.recovery,f.combo,f.streak].every(finite)&&f.maxStamina>0&&f.injuryLimit>0&&f.stamina<=f.maxStamina&&f.injury<=f.injuryLimit&&f.combo<=DUEL_RULES.comboCap&&f.streak<=DUEL_RULES.streakCap&&f.build&&f.build.war>=1&&f.build.war<=100&&f.build.lead>=1&&f.build.lead<=100&&validDuelBuild(f.build)&&typeof f.taunted==='boolean'&&typeof f.fainted==='boolean'&&action(f.previous)&&DUEL_ACTIONS.every(a=>finite(f.points[a])&&f.points[a]<=DUEL_RULES.pointsCap);
...
export function validEncounterProgress(value:unknown):value is EncounterProgress {
 ...
 return validContest(e.contest);
 }catch{return false;}
}
export function validContest(value:unknown):boolean {
 try {
 const c=value as EncounterProgress['contest'];if(!c)return c===null;
