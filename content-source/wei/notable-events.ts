...
/**
 * 人物事件的功績量級 ★ 對照【委託】的 3／10／18（core/events/commissions.ts）
 *
 * 一則人物事件一輪只出現七次，委託十四次、固定事件二十八次，
 * 所以單則值更多是對的 —— 但實測它值得【太多】：
 *
 *   每次功績　固定事件 6.4　委託 19.4　**人物事件 40.0**
 *
 * 兩個放大器疊在一起：稀有度來自名士本人（3–5 星 → ×1.3–1.7，
 * 而委託的稀有度來自光階，多半 ×1.0–1.15），加上階級倍率原本吃兩線較高者。
 * 後者已由 D69 修掉（改吃收款線），這裡再把量級本身拉回委託的尺上。
 */
const SMALL = 5;
const MID = 11;
const BIG = 20;
