...
export const itemTexts: Record<string, string> = {
  ...
  'item.xiaoyaojin.tier.0': '張遼出現在統御格的權重 10 → 40',
  'item.xiaoyaojin.tier.1': '張遼的好感成長 +150%',
  'item.xiaoyaojin.tier.2': '該格只有一位名士時，整格養成 +100%',
  'item.xiaoyaojin.tier.3': '張遼的同框加成再 +100%',
  'item.xiaoyaojin.tier.4': '統御的成長量 +40%',
  'item.xiaoyaojin.tier.5': '該格只有一位名士時，整格養成再 ×2（合計 ×4）',
