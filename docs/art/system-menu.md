...
- 選單：500px 寬，依內容高度排列 406×98px 長條按鈕。進行中的本輪依序顯示系統設定、返回主選單、放棄本輪、離開遊戲、繼續遊戲；放棄與離開均使用朱紅令牌。無本輪或已進入結局時不顯示放棄。外框以 9-slice 延展，令牌使用原美術的獨立 CSS 圖集切片，Hover/focus 只描亮外緣。
