...
> **事上磨練（經驗）與功績用【兩套不同的縮放】**（`practiceTierMul` /
> `practiceRarityMul` 對 `tierMultiplier` / `rarityMultiplier`）。
> 功績要追得上官階門檻（6405），經驗對照的是 0–100 的四維與一輪約 900 的總量。
> 共用一條的話，一則 ★4 事件會給掉一整個等級 —— 那是舊制
> （產出即屬性、上限 999）留下來的尺度錯誤。
