...
> | 貨幣 | 只吃 | 一則 ★N 中檔委託 |
> |---|---|---|
> | 經驗 | **稀有度**（`practiceRarityMul` ＝ `[1,2,3,4,5]`） | `30 × N` |
> | 功績 | **官階**（`tierMultiplier` → 7.6） | `10 × 官階倍率 × 1.15–1.7` |
>
> 「經驗值總值 ＝ 基礎事件值 × 星數」是玩家訂的規矩，所以那張表
> 就是星數本身。舊制兩者都吃兩條鏈（相乘 178 倍），於是同一個決定
> 同時放大兩種貨幣，「追光階 對 追驚嘆號」讀不出差別（實測只差 5.2%）。
>
> 分開之後那個取捨才有內容：**光階買經驗，委託旗標買功績。**
> `practiceTierMul`（經驗的官階項）已刪除 —— 經驗的官階成長由鍛鍊與
> 戰役深度承擔，事件不必再疊一層看不見的乘數。
...
### 6.3 檢定失敗 ＝ **顆粒無收**（`failRatio = 0`）★★

`ratio = passed ? 1 : failRatio`，而 `failRatio` 現在是 **0**。

**舊值 0.4 的理由是「若失敗＝顆粒無收，玩家永遠只會選最穩的選項」。
實測反過來：0.4 之下玩家永遠只會選最兇的。**
...
**高檔失敗的 36 還比低檔成功的 15 多兩倍** —— 於是高檔怎麼算都贏，
三檔又退化成一檔。改成 0 之後期望變成 15／25／22：
**順序由你的四維決定**，四維夠高高檔才划算，不夠就該退回中檔。

而互斥制度那個顧慮由另一件事承擔：**低檔的 DC 改成幾乎必過**
（18 §3，0.50 倍 → 99%）。想穩就選低檔，它保證有東西；
想要多就得押上這一回合。兩者都成立，才是一條階梯。
...
5. 小檢定失敗**不寫入 `RunState.ending`**（戰役的戰敗現在也不會 —— 33 §6.4），且產出為零（`failRatio = 0`）
