...
**成本區間與等級邊界對齊是刻意的**：玩家看到「武 B」就知道下一階每點要付 50。
七個價格帶 ＝ 七個等級，不需要額外教學，也不需要在 UI 另外解釋成本曲線。

> **實測支撐**（13 策略 × 300 輪）：一輪總經驗 **2691–4765**。
> 專精者主維收在 **93–95**、其餘三維停在起始值附近（28–30）；
> 均衡者收在 **統82 武82 智55 政49**。
> 所有策略都剩下 **471–692 未花（總量的 15–18%）—— 買不完。**
>
> 於是這張表要產出的那個決策是真的：
> **S 級（1408）需近乎全押** —— 只有 risk-averse 摸到 武95，特質只有 2.8；
> **A 級（908）留得下約 500 給特質** —— notable-chaser 收在 88／88 帶 5.4 條特質。
>
> 這裡的「近乎全押」是字面的：`risk-averse` 的一輪總經驗 2914，
> 而從起始值練到 95 就要 1364。
...
實測（13 策略 × 300 輪）：專精收在 **武93–95、特質 2.7**；
均衡收在 **統82 武82、特質 4.2**；投資關係的收在 **88／88、特質 5.4**。
「S 級空手」與「A 級帶特質」兩端都真的走得通，而且**沒有一端支配另一端**
（點數 9239–11162，差 21%）。

### 4.2 機會成本的驗算

| 玩家站在 | 每點數值 | 一個常階特質（100）等於 | 一個絕階（450）等於 |
|---|---|---|---|
| A 帶（專精） | 50 | 2 點數值 | 買不起（沒有另外兩類） |
| C 帶（均衡） | 18 | 6 點數值 | 25 點數值 |
