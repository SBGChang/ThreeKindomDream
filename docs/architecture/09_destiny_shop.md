...
商店賣的東西**有一半在介面層沒有出口** —— `startRun` 直接吃 `emptyDraft`，
於是資質配點、天賦、攜帶道具、指定玩伴買了都不能用。

那比任何數值問題都嚴重：**數值層一直是對的，斷的是那座橋。**
現在四樣都接在 [14 入夢配置](14_dream_entry_config.md) 的畫面上。

| 商店賣的 | 出口 |
|---|---|
| 官途 → 官階上限 | 入夢時定案（不用選，直接算進 `config.careerCap`） |
| 資質上限 ＋ 資質配點 | **入夢畫面**：四維各挑一階，砍低一維可退點 |
| 天賦解放 ＋ 配帶點數 | **入夢畫面**：從已解放的池裡挑到點數用完 |
| 勢力緣分 → 指定名額 | **入夢畫面**：挑玩伴 |
| 升階機率 | 局內自動生效 |
| （碎片 → 名士星階） | 天命畫面的名士圖鑑 |
| （碎片 → 道具階級） | 天命畫面的**道具圖鑑**；攜帶在入夢畫面 |

**攜帶那一條是道具跨輪成長的唯一入口**：高階道具 `perRunCap = 1`，
那唯一一次是「首次獲得」而不是重複 —— **不帶進場就永遠拿不到碎片**（23 §5）。

（局內【持有中】的道具也看得見了：狀態列的「隨身」列，含效果說明。）
