...
而正因為它只是資料，**主角可以不吃這個對應**（RFC-01 D19）。內容裡因此有兩條
「越界」的常階招，職責是給統與政各一條自己的輸出路：

| 招 | `actorAttr` | `kind` | 為什麼 |
|---|---|---|---|
| 〈號令〉 | 統 | 物理 | 你不親自砍，你叫別人砍 |
| 〈亂辭〉 | 政 | 法術 | 以言辭亂其軍心，那也是傷害 |

兩條都比同維本行招弱一截（0.24／0.26 對 0.30／0.32）——
**四條路都能打，但每條路都有它更擅長的那一手。**
名士不靠它們定位：它們掛在 `star 1` 以上，名士的 `star-0` 一定是本行那一招。
...
| **每一維都有至少一條常階輸出招**（`physical` 或 `magic`） | `ability.starterSkill()` 拿起始四維最高那一維去找它。找不到就回 `null` —— 玩家帶著三個空格走進第一場戰役 |

**刻意沒有**「`kind` 與 `actorAttr` 的對應」這一條。四職能對應
（武物理／智法術／政恢復／統 Buff）**約束名士的定位，不約束 def 的性質**——
名士的定位寫在他的 `star-0` 招裡（那一定是他本行）。
主角不受此限（RFC-01 D19）：他的四維決定他**怎麼打**，不決定他在隊裡的功能。
所以〈號令〉是統的**物理**招、〈亂辭〉是政的**法術**招，兩條都合法。

最後一條是這張表裡唯一擋得住真實事故的：統與政原本一條輸出招都沒有，
七道驗證全綠、81 項斷言全過，只有真的去玩才看得出來（RFC-01 D46）。
