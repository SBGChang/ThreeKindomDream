...
$battleOutput=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../public/art/units/battle-demo'))
...
using System.IO;
public static class BattleArtImport {
...
'@
foreach($battleClip in @('run','slash','hurt','mounted','drum','enemy-run','enemy-slash','enemy-hurt')){
 $battleRows=if($battleClip.EndsWith('run')){4}else{2}
 [BattleArtImport]::Run([IO.Path]::GetFullPath((Join-Path $PSScriptRoot "../docs/art/sources/battle-demo/$battleClip.png")),$battleOutput,$battleClip,$battleRows)
}
