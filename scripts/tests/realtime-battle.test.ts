...
const s=createBattle(425);assert.equal(armyCount(s,'ally'),425);assert.equal(s.units.filter(u=>u.side==='ally').length,10);assert.ok(s.units.every(u=>u.hp<=50));
...
assert.equal(large.units.filter(u=>u.side==='ally').length,50);
