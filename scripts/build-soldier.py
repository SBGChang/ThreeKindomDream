...
from soldier_motion import refine_motion
...
for idx,(o,kind,bone_name) in enumerate(parts):
    ...
    for target,pal,faction in [(base,palette,'host'),(enemy_base,{**palette,**enemy_palette},'enemy')]:
        ...
        # Continuous limb islands: paint cuffs across triangles instead of assigning
        # a different tile per polygon (which produces saw-tooth skin/binding edges).
        if o.name.startswith('Sleeve'):
            ink(u>.68,palette['skin'][:3]);ink((u>.635)&(u<.66),tuple(c*.60 for c in color[:3]))
        if o.name.startswith('Short trousers'):
            ink(v<.43,palette['bindings'][:3])
            ink((v<.43)&(v>.22)&(((v*35)%1)<.18),(.23,.14,.075))
            ink(v<.22,palette['leather'][:3])
    ...
    if o.name.startswith('Spear'):
        tag=o.data.attributes.new('spear_vertices','BOOLEAN','POINT')
...
clips={}
for name,end in [('Idle',48),('Run',24),('Thrust',32),('Hit',20),('Death',20),('Cheer',40)]:
...
refine_motion(body,rig,clips,chibi_point,arm_maps)
...
bpy.ops.object.camera_add();camera=bpy.context.object;scene.camera=camera;camera.data.type='ORTHO';camera.data.ortho_scale=3.35
def camera_at(loc,target=(0,0,1.35)):
...
for label,loc in [('front',(0,-6,1.2)),('side',(6,0,1.2)),('back',(0,6,1.2)),('hero',(3,-5,2.5))]:
    camera_at(loc if label=='hero' else (loc[0],loc[1],1.35));scene.render.filepath=str(OUT/(label+'.png'));bpy.ops.render.render(write_still=True)
...
body.hide_render=True;camera.data.ortho_scale=3.35;camera_at((0,-6,1.35));scene.render.filepath=str(OUT/'connected-body.png');bpy.ops.render.render(write_still=True)
body.hide_render=False;bpy.data.objects.remove(core_preview,do_unlink=True);bpy.data.meshes.remove(core_mesh)
rig.data.pose_position='REST';scene.render.filepath=str(OUT/'t-pose.png');bpy.ops.render.render(write_still=True);rig.data.pose_position='POSE';camera.data.ortho_scale=3.35
color_node.image=enemy_albedo
...
camera_at((-3,-5,2.3));camera.data.ortho_scale=3.55
sprite_meta={}
for faction,texture in [('host',albedo),('enemy',enemy_albedo)]:
    color_node.image=texture
    for name,(action,end) in clips.items():
        camera.data.ortho_scale=4.5 if name=='Cheer' else 3.55
        camera_at((-3,-5,2.8) if name=='Cheer' else (-3,-5,2.3),target=(0,0,1.85) if name=='Cheer' else (0,0,1.35))
...
(OUT/'manifest.json').write_text(json.dumps({'style':'approved-long-round-body','vertices':len(body.data.vertices),'triangles':len(body.data.loop_triangles),'bones':len(rig.data.bones),'atlas':N,'uvParts':len(uv_info),'bodyConnectedComponents':1,'factions':['host','enemy'],'animations':sprite_meta},indent=2))
